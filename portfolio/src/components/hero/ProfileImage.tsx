...
const ProfileImage: React.FC<ProfileImageProps> = ({size, className = ''}) => {
    ...
    return (
        <div className={`relative ${className}`}>
            ...
            {/* Glowing ring effect */}
            <div
                className="absolute -inset-4 rounded-full border-2 border-primary/20 animate-spin animate-duration-8000"></div>
            <div
                className="absolute -inset-8 rounded-full border border-accent/10 animate-spin animate-duration-12000 animate-reverse"></div>
        </div>
    );
};
